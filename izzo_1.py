...
from assertions_1 import assert_valid_parameters  # verify function input parameters

# from numba import njit as jit # jit = numba code accelerator; imporve execution performance


# @jit # jit = numba code accelerator; imporve execution performance
def izzo2015(
    mu,
    r1,
    r2,
    ...
    Input Parameters:
    ----------
        mu       : float, Gravitational parameter of attractor body.
        ...
        prograde : bool, true= prograde motion, otherwise retrograde.
        low_path : bool, if two solutions available, select between high or low path.
    ...
    Notes
    ----------
        Algorithm devised by Dario Izzo[1] in 2015. It inherits from the one
        developed by Lancaster[2] during the 60s, following the universal
        formulae approach. It is one of the most modern solvers, being a
        complete Lambert's problem solver (zero and Multiple-revolution
        solutions). It shows high performance and robustness while requiring
        no more than four iterations to reach a solution.
    ...
    assert_valid_parameters(mu, r1, r2, tof, M)
    ...
    # Compute radial and tangential components at initial and final
    #   position vectors
...
def norm(vector):
    ...
    Input Parameters:
    ----------
        vector : ~np.array, vector whose magnitude is to be computed.
...
def _halley(p0, T0, ll, atol, rtol, maxiter):
    ...
    Notes:
    ----------
        This function is private because it assumes a calling convention
